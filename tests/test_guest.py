...
async def test_guest_rated_movies(get_data, assert_data):
    ...
    with patch("themoviedb.routes._base.ClientSession.request") as mocked:
        ...
        rated_movies = await tmdb.TMDb().guest(guest_session_id).rated_movies()
...
async def test_guest_rated_tvs(get_data, assert_data):
    ...
    with patch("themoviedb.routes._base.ClientSession.request") as mocked:
        mocked.return_value.__aenter__.return_value.json.return_value = data
        rated_tvs = await tmdb.TMDb().guest(guest_session_id).rated_tvs()
        mocked.assert_called_with(
            "GET",
...
async def test_guest_rated_episodes(get_data, assert_data):
    ...
    with patch("themoviedb.routes._base.ClientSession.request") as mocked:
        mocked.return_value.__aenter__.return_value.json.return_value = data
        rated_episodes = await tmdb.TMDb().guest(guest_session_id).rated_episodes()
        mocked.assert_called_with(
            "GET",
