...
async def test_movie_details_full(get_data, assert_data):
    ...
    with patch("themoviedb.routes_async._base.ClientSession.request") as mocked:
        mocked.return_value.__aenter__.return_value.json.return_value = data
        movie = await aiotmdb.aioTMDb().movie(movie_id).details(append_to_response="alternative_titles,changes,credits,external_ids,images,keywords,lists,recommendations,release_dates,reviews,similar,translations,videos,watch/providers")
        mocked.assert_called_with(
            "GET",
            f"https://api.themoviedb.org/3/movie/{movie_id}",
            params={
