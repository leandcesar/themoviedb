...
def test_guest_rated_movies(get_data, assert_data):
    ...
    with patch("themoviedb._routes.sync._base.Session.request") as mocked:
        mocked.return_value.__enter__.return_value.json.return_value = data
        rated_movies = tmdb.TMDb().guest(guest_session_id).rated_movies(sort_by="created_at.asc")
...
def test_guest_rated_tvs(get_data, assert_data):
    ...
    with patch("themoviedb._routes.sync._base.Session.request") as mocked:
        mocked.return_value.__enter__.return_value.json.return_value = data
        rated_tvs = tmdb.TMDb().guest(guest_session_id).rated_tvs(sort_by="created_at.desc")
        mocked.assert_called_with(
            "GET",
...
def test_guest_rated_episodes(get_data, assert_data):
    ...
    with patch("themoviedb._routes.sync._base.Session.request") as mocked:
        mocked.return_value.__enter__.return_value.json.return_value = data
        rated_episodes = tmdb.TMDb().guest(guest_session_id).rated_episodes(sort_by="created_at.asc")
        mocked.assert_called_with(
            "GET",
