...
async def test_guest_rated_movies(get_data, assert_data):
    ...
    with patch("themoviedb._routes.async_._base.ClientSession.request") as mocked:
        ...
        rated_movies = await aiotmdb.aioTMDb().guest(guest_session_id).rated_movies(sort_by="created_at.asc")
...
async def test_guest_rated_tvs(get_data, assert_data):
    ...
    with patch("themoviedb._routes.async_._base.ClientSession.request") as mocked:
        mocked.return_value.__aenter__.return_value.json.return_value = data
        rated_tvs = await aiotmdb.aioTMDb().guest(guest_session_id).rated_tvs(sort_by="created_at.desc")
        mocked.assert_called_with(
            "GET",
...
async def test_guest_rated_episodes(get_data, assert_data):
    ...
    with patch("themoviedb._routes.async_._base.ClientSession.request") as mocked:
        mocked.return_value.__aenter__.return_value.json.return_value = data
        rated_episodes = await aiotmdb.aioTMDb().guest(guest_session_id).rated_episodes(sort_by="created_at.asc")
        mocked.assert_called_with(
            "GET",
